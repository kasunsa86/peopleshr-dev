// PeoplesHR News page — fetches posts from the "News" category (id 19)
// live from the WordPress REST API and renders them using the same
// .cs-grid / .cs-card styling as the blog, events, and case-studies pages.
// New posts published on WordPress under the News category appear here
// automatically — nothing on this page needs manual updates.
...
(function () {
  ...
  var errorBox = document.getElementById('newsError');
  var loadMoreRow = document.getElementById('newsLoadMoreRow');
  var loadMoreBtn = document.getElementById('newsLoadMoreBtn');
  ...
  function cardHTML(post) {
    ...
    // Always label "News" here — this page is filtered to the News category,
    // but posts are often cross-tagged (e.g. also under Events), so the
    // first embedded term isn't reliably "News".
    var category = 'News';
  ...
  function fetchPage(p) {
    return fetch(API_BASE + '?_embed&categories=' + NEWS_CATEGORY_ID + '&per_page=' + PER_PAGE + '&page=' + p).then(function (res) {
